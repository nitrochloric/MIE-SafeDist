...
static float temp;
static IKS01A3_MOTION_SENSOR_Axes_t Axes;
//I2C_HandleTypeDef hi2c1;
//MMA7660_Handle sensor;
/* USER CODE END PV */
...
void BUZZER_control(int select);
/* USER CODE END PFP */
...
int main(void)
{
  ...
  if(IKS01A3_ENV_SENSOR_SetOutputDataRate(IKS01A3_STTS751_0, ENV_TEMPERATURE, 4.0f) != BSP_ERROR_NONE){
	  printf("error of output data rate setting\r\n");
  }
  if(IKS01A3_MOTION_SENSOR_Init(IKS01A3_LIS2DW12_0, MOTION_ACCELERO) != BSP_ERROR_NONE){
  	  printf("error of initialization\n\r");
    }

    if(IKS01A3_MOTION_SENSOR_SetOutputDataRate(IKS01A3_LIS2DW12_0, MOTION_ACCELERO, 4.0f) != BSP_ERROR_NONE){
  	  printf("error of output data rate setting\r\n");
    }


  	//MMA7660_setSampleRate(sensor, AUTO_SLEEP_32);
  /* USER CODE END 2 */
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOG, BUZZER_Pin|MCU_ACTIVE_Pin, GPIO_PIN_RESET);
  ...
  /*Configure GPIO pins : BUZZER_Pin MCU_ACTIVE_Pin */
  GPIO_InitStruct.Pin = BUZZER_Pin|MCU_ACTIVE_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_VERY_HIGH;
...
/* USER CODE BEGIN 4 */
float ACCEL_SENSOR_GetValue(){
	//MMA7660_getXYZ(sensor, &ax,&ay,&az);
	//float ax,ay,az;
	IKS01A3_MOTION_SENSOR_GetAxes(IKS01A3_LIS2DW12_0, MOTION_ACCELERO, &Axes);
	printf("ax = %f ay = %f az = %f\r\n", (float)Axes.x/100, (float)Axes.y/100, (float)Axes.z/100);
	return Axes.x;
}

void BUZZER_control(int select){
	HAL_GPIO_WritePin(BUZZER_GPIO_Port, BUZZER_Pin, select);
}

uint32_t TEMP_SENSOR_GetValue(){
	IKS01A3_ENV_SENSOR_GetValue(IKS01A3_STTS751_0, ENV_TEMPERATURE, &temp);
	return temp;
}
